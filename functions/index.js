...
// https://us-central1-newagent-1185b.cloudfunctions.net/helloWorld
...
app3.get("*", (request, response) => {
  if (request.query.location) {
    response.send(
      circularJson.stringify(findEvents(request.query.location))
    )
  } else {
    response.send('error mang')
  }
})
...
const findEvents = (location) => {
  const eventBriteToken = 'EP5XIBBB4YAVJ2FZDHAE';

  requestNode('https://www.eventbriteapi.com/v3/events/search/?token='+ eventBriteToken + '&location.address="' + location + '"', function (error, response, body) {
    if (error) {
      return 'false';
    }
    if (response) {
      if (response.events.length) {
        return 'we got some events'
